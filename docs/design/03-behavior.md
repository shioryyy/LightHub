...
界面分别显示“本地已保存”“已写入指定板载槽”“当前生效”，允许这些状态不同。宏本地保存不代表按键已绑定、板载槽已启用或运行器已启动；绑定也须显示执行来源以及关闭软件后是否有效。板载不支持时仅给出原因和软件方案入口，不自动转换。

固定软件配置只由用户显式选择，前台应用变化不得改变配置、DPI、按键或执行中的宏。以后开启应用规则时，显示当前配置、来源和最近切换原因，提供一键锁定；未匹配应用使用用户指定默认配置，不使用隐式出厂配置。切换先停止旧宏、释放持有按键、清空触发队列，再切换配置。

运行器状态独立为 Off、Starting、Ready、Running、Stopping、Faulted，不复用设备 Writing 状态。编辑已启动配置仅修改草稿，采用不可变执行快照；新版本须显式启用。Stop 停止接收触发、取消延时和待执行事件、释放运行器持有按键；断连、权限丢失、休眠和注入失败同样停止且清空队列。重连/唤醒不重放旧事件，首批要求用户重新启用。

默认关闭窗口停止运行器并退出。明确选择继续运行时显示托盘状态和停止入口；“全部退出”必须停止运行器。异常终止后的系统按键状态存在平台限制，不能承诺绝对释放；重启不恢复旧执行。详细验收见 R01—R06。
